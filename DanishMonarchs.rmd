...
separator: kings.csv are sepperated by ","
...
2 and 4 are tidyverse functions
...
for "csv" functions, there are 11 columns, where as the "csv2" functions only show 1 column

4. Show the dataset so that we can see how R interprets each column
using the glimpse function, we can see each column and its type of data
...
```{r eval=TRUE}
#First i'm doing R's "mean()" function
kings_with_duration <- kings %>% 
  filter(!is.na(End_year)) %>% 
  filter(!is.na(Start_year)) %>% 
   mutate(duration=End_year-Start_year)
reign_duration <- kings_with_duration$duration
mean(reign_duration)


#You can also do it with the "summarize()" function from tidyverse
kings %>% 
  filter(!is.na(End_year)) %>% 
  filter(!is.na(Start_year)) %>% 
  mutate(duration=End_year-Start_year) %>% 
...
#Both come to a mean of 19.5 years
...
kings_long_reign <- kings_with_duration %>%
  ...
  filter(duration>19.5)
kings_long_reign
count(kings_long_reign)
# 25 monarchs had a longer than average reign
```
...
kings_with_duration %>% 
  ...
  select(Name,Start_year,End_year,duration, duration_days)
```
